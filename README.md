...
.pm-section{padding:8px 16px;border-bottom:1px solid var(--border);}
.pm-bar-row{display:flex;align-items:center;gap:6px;margin-bottom:3px;}
.pm-lbl{font-family:var(--mono);font-size:8px;width:22px;flex-shrink:0;}
.pm-bar{flex:1;height:5px;background:var(--surface2);border-radius:3px;overflow:hidden;}
.pm-fill{height:100%;border-radius:3px;}
.pm-pct{font-family:var(--mono);font-size:9px;width:26px;text-align:right;flex-shrink:0;}
.pm-sig{display:inline-flex;align-items:center;gap:3px;padding:2px 8px;border-radius:10px;font-family:var(--mono);font-size:9px;margin-top:4px;}
.pm-sig.beat{background:var(--green-bg);color:var(--green);}
.pm-sig.miss{background:var(--red-bg);color:var(--red);}
.pm-sig.warn{background:var(--gold-bg);color:var(--gold);}
.pm-sig.n{background:var(--surface2);color:var(--ink3);}
.pm-none{font-family:var(--mono);font-size:9px;color:var(--ink3);text-align:center;padding:4px 0;}
.vw-badge{font-family:var(--mono);font-size:8px;padding:1px 5px;border-radius:3px;margin-left:4px;}
.vw-badge.h{background:var(--green-bg);color:var(--green);}
.vw-badge.m{background:var(--gold-bg);color:var(--gold);}
.vw-badge.l{background:var(--red-bg);color:var(--red);}
...
<header>
  <div>
    ...
    <div class="logo-sub">21 Mega Caps · 🦁 5 Berkshire Holdings · Powered by Cloudflare</div>
  </div>
...
<div class="summary-bar">
  <div class="s-stat"><div class="s-lbl">Stocks</div><div class="s-val" id="sTotal">21</div></div>
  ...
  <div class="s-stat"><div class="s-lbl">Earnings Active</div><div class="s-val" id="sEarnings">—</div></div>
  <div class="s-stat"><div class="s-lbl">Beat Signal</div><div class="s-val g" id="sBeat">—</div></div>
</div>
<div class="filter-bar">
  ...
  <button class="f-btn" onclick="setFilter('earnings',this)">🎯 Earnings Active</button>
</div>
...
<script>
const WORKER = 'https://small-truth-5384.jgschneider.workers.dev';
...
// 21 stocks confirmed working on FMP free tier
// 🦁 marks Berkshire Hathaway holdings
const STOCKS = [
  // Tech (7)
...
// ── Fetch via Cloudflare Worker proxy (handles CORS + auth) ──────────────────
async function fmpFetch(endpoint, symbol){
  const url = `${WORKER}/?endpoint=${endpoint}&symbol=${symbol}`;
...
async function fetchPolymarket(ticker, name) {
  try {
    const q = `${ticker} earnings`;
    const url = `https://gamma-api.polymarket.com/events?active=true&limit=5&keyword=${encodeURIComponent(q)}`;
    const res = await fetch(url, { headers: { 'Accept': 'application/json' } });
    if (!res.ok) return null;
    const data = await res.json();
    if (!Array.isArray(data) || !data.length) return null;
    // Find most relevant market
    const rel = data.find(e => (e.title||'').toLowerCase().includes(ticker.toLowerCase()));
    return rel || data[0];
  } catch(e) { return null; }
}

function parsePM(event) {
  ...
  const yi = outcomes.findIndex(o => o.toLowerCase().includes('yes') || o.toLowerCase().includes('beat'));
  if (yi < 0 || prices[yi] == null) return null;
  const yesPct = Math.round(prices[yi] * 100);
  const vol    = parseFloat(m.volume || 0);
  const liq    = parseFloat(m.liquidity || 0);
  const dist   = Math.abs(yesPct - 50);
  // VW Confidence Score
  let vw = 0;
  vw += vol>=500000?40:vol>=200000?35:vol>=100000?28:vol>=50000?20:vol>=10000?10:vol>=1000?4:0;
  vw += dist>=15&&dist<=35?35:dist>=10?25:dist>42?5:10;
  vw += liq>=100000?15:liq>=50000?10:liq>=10000?5:0;
  if (dist > 40 && vol < 50000) vw -= 20; // SOFI overconfidence penalty
  vw = Math.max(0, Math.min(100, vw));
  const daysOut = m.endDate ? Math.round((new Date(m.endDate) - Date.now()) / 86400000) : null;
  return { yesPct, vol, vw, daysOut, sofi: dist > 40 && vol < 50000 };
}
...
async function loadStock(stock){
  ...
  // All calls go through Worker — no CORS issues, no rate limits
  try{
    const d=await fmpFetch('quote', t);
  ...
  }catch(e){ setStatus(`${t} quote: ${e.message}`); }

  try{
    const d=await fmpFetch('key-metrics-ttm', t);
    metrics=Array.isArray(d)?d[0]:d;
  }catch(e){ setStatus(`${t} metrics: ${e.message}`); }

  try{
    const d=await fmpFetch('ratios-ttm', t);
    ratios=Array.isArray(d)?d[0]:d;
  }catch(e){ setStatus(`${t} ratios: ${e.message}`); }

  try{
    const d=await fmpFetch('dividends', t);
    divs=Array.isArray(d)&&d.length?d:null;
  }catch(e){ /* dividends optional */ }

  // Polymarket — best effort, won't block if it fails
  let pmOdds = null;
  try { pmOdds = await fetchPolymarket(stock.t, stock.n); } catch(e) {}

  return {quote, metrics, ratios, divs, pmOdds};
}
...
function renderCard(d){
  ...
  const pe=q.pe||m.peRatioTTM||m.peRatio;
  const pb=m.pbRatioTTM||m.priceToBookValueTTM||r.priceToBookRatioTTM||r.priceToBook;
  const roe=(m.roeTTM||m.returnOnEquityTTM||r.returnOnEquityTTM||0)*100;
  const evEbitda=m.evToEBITDATTM||m.enterpriseValueOverEBITDATTM||m.evToEbitdaTTM;
  const evSales=m.evToSalesTTM||m.enterpriseValueToSalesTTM;
  const cr=m.currentRatioTTM||m.currentRatio;
  const gpm=r.grossProfitMarginTTM||r.grossProfitMargin;
  const npm=r.netProfitMarginTTM||r.netProfitMargin;
  const opm=r.operatingProfitMarginTTM||r.operatingProfitMargin;
  const fcfY=(m.freeCashFlowYieldTTM||m.freeCashFlowYield||0)*100;
  const de=m.debtToEquityTTM||m.debtToEquity||r.debtEquityRatioTTM||r.debtToEquity;
  ...
  <div class="fund-grid">
    ...
    <div class="fund-item"><div class="f-lbl2">EV/Sales</div><div class="f-val ${evSales?(evSales<3?'g':evSales<8?'n':'r'):'n'}">${evSales?f1(evSales):'—'}</div></div>
    <div class="fund-item"><div class="f-lbl2">FCF Yield</div><div class="f-val ${fcfY>5?'g':fcfY>2?'n':'n'}">${fcfY?f1(fcfY)+'%':'—'}</div></div>
    <div class="fund-item"><div class="f-lbl2">Current Ratio</div><div class="f-val ${cr?(cr>1.5?'g':cr>1?'n':'r'):'n'}">${cr?f2(cr):'—'}</div></div>
  ...
  <div class="sec-title">POLYMARKET EARNINGS ODDS</div>
  <div class="pm-section">${(function(){
    const pm = parsePM(d.pmOdds);
    if (!pm) return '<div class="pm-none">No active earnings market on Polymarket</div>';
    const no = 100 - pm.yesPct;
    const vwL = pm.vw>=65?{t:'HIGH CONF',c:'h'}:pm.vw>=40?{t:'MODERATE',c:'m'}:{t:'LOW VOL',c:'l'};
    const pSig = pm.yesPct>=75?'beat':pm.yesPct<=40?'miss':pm.sofi?'warn':'n';
    const pTxt = pm.yesPct>=75?pm.yesPct+'% BEAT EXPECTED':pm.yesPct<=40?no+'% MISS RISK':pm.sofi?'⚠ OVERCONF FLAG':'UNCERTAIN';
    const dStr = pm.daysOut!=null?(pm.daysOut<=0?'resolving':pm.daysOut+'d away'):'';
    return '<div class="pm-bar-row"><div class="pm-lbl" style="color:var(--green)">YES</div><div class="pm-bar"><div class="pm-fill" style="width:'+pm.yesPct+'%;background:var(--green-light)"></div></div><div class="pm-pct" style="color:var(--green)">'+pm.yesPct+'%</div></div>'
      +'<div class="pm-bar-row"><div class="pm-lbl" style="color:var(--red)">NO</div><div class="pm-bar"><div class="pm-fill" style="width:'+no+'%;background:#e07050"></div></div><div class="pm-pct" style="color:var(--red)">'+no+'%</div></div>'
      +'<div style="display:flex;align-items:center;justify-content:space-between;margin-top:4px"><span class="pm-sig '+pSig+'">'+pTxt+'</span><div><span class="vw-badge '+vwL.c+'">'+vwL.t+' '+pm.vw+'/100</span>'+(dStr?'<span style="font-family:var(--mono);font-size:8px;color:var(--ink3);margin-left:4px">'+dStr+'</span>':'')+'</div></div>'
      +(pm.sofi?'<div style="font-family:var(--mono);font-size:8px;color:var(--red);margin-top:3px">⚠ Extreme odds + thin volume — see SOFI Apr 2026 case</div>':'');
  })()}</div>
...
function renderCards(){
  ...
  else if(activeFilter==='earnings') cards=cards.filter(d=>parsePM(d.pmOdds)!=null);
...
function updateSummary(){
  let buys=0,watches=0,grahams=0,divs=0,earnings=0,beats=0;
  allData.filter(d=>d.metrics).forEach(d=>{
    ...
    const pm=parsePM(d.pmOdds);
    if(pm){ earnings++; if(pm.yesPct>=75&&pm.vw>=45) beats++; }
  });
  ...
  document.getElementById('sEarnings').textContent=earnings;
  document.getElementById('sBeat').textContent=beats;
}
  ...
  for(let i=0;i<STOCKS.length;i++){
    ...
    try{
      ...
      if(idx>=0){
        ...
        allData[idx].pmOdds  = result.pmOdds;
    ...
    // Small gap between stocks — Worker handles rate limiting server-side
    if(i<STOCKS.length-1) await delay(500);
  }
